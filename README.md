Vanilla JS web application for viewing Finnish student restaurants and their daily and weekly menus.

## Student info

Kseniia Shlenskaia  
Tieto-ja viestintätekniikka  
TVT25K-O  
Ohjelmistotuotanto

## Requirements

- [ ] Restaurants are listed
- [ ] The user can choose whether to view the daily or weekly menu
- [ ] Daily and weekly menus are displayed
- [ ] The application is published on `users.metropolia.fi` or another public server
- [x] The source code is available on GitHub or a similar service
- [x] Good Git practices are used
- [ ] User login and registration
- [ ] The user can select favourite restaurants
- [ ] The user can update profile information
- [ ] The user can add a profile image
- [ ] Restaurants can be filtered by city, provider, or other criteria
- [ ] Restaurants are listed clearly and logically
- [ ] Restaurants are shown on a map
- [ ] The nearest restaurant is highlighted
- [ ] Additional custom functionality
- [ ] Front-end application logic is implemented with vanilla JavaScript
- [ ] ES6+ features and JavaScript best practices are used
- [x] No React, Angular, jQuery, or similar front-end libraries are used
- [x] No CSS frameworks such as Bootstrap are used
- [x] Good programming practices are followed
- [x] Naming is logical and consistent
- [x] Indentation and formatting are clean
- [ ] HTML and CSS are professionally implemented and validated
- [x] The UI is clean and clear
- [x] Responsive design is implemented

## Structure

```text
├── index.html
├── pages/
│   ├── favourites.html
│   ├── map.html
│   └── profile.html
├── assets/
│   ├── icons/
│   └── styles/
└── README.md
```
